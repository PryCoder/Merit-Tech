        ...
        >
          <Box>
            ...
            <Text mt={2} color="blackAlpha.700" maxW="70ch">
              High-level metrics, active job postings, and a real-time feed of
        ...
        <Stack spacing={8}>
          ...
          >
            ...
            {!isLoading && assessments.length === 0 ? (
              <Text color="blackAlpha.600">No active postings.</Text>
            ) : null}
                  ...
                  <HStack justify="space-between" align="start">
                    <Box>
                      ...
                      >
                        ...
                        <Text fontFamily="var(--font-geist-mono)">
                          ID: {a.id}
                        </Text>
                      </HStack>
                    </Box>
          ...
          >
            ...
            >
              ...
              <Button
                variant="outline"
                onClick={() => refreshFeed(assessments)}
            ...
            {feed.length === 0 ? (
              ...
              <Stack spacing={3}>
                {feed.map((item) => (
                  <Box
                    key={`${item.sessionId}:${item.submittedAt}`}
                    borderWidth="1px"
                    borderColor="blackAlpha.100"
                    borderRadius="xl"
                    p={4}
...
function CandidateDashboard({ me }: { me: MeResponse['user'] }) {
  const [data, setData] = React.useState<CandidateDashboardResponse | null>(
    null
  );
  const [isLoading, setIsLoading] = React.useState(true);
  const [error, setError] = React.useState<string | null>(null);
  ...
  async function refresh() {
    ...
    setError(null);
    try {
      const res = await fetchJson<CandidateDashboardResponse>(
      ...
      <Container maxW="6xl" py={{ base: 10, md: 14 }}>
        ...
        >
          <Box>
            ...
            <Text mt={2} color="blackAlpha.700" maxW="70ch">
              Pending assessment invites, completed challenges, and your Merit
              Score progression.
            </Text>
          </Box>
                      ...
                      >
                        <Box>
                          ...
                          <Text
                            ...
                            fontFamily="var(--font-geist-mono)"
                          >
                            {new Date(c.submittedAt).toLocaleString()} — Session{' '}
                            {c.sessionId}
                          </Text>
