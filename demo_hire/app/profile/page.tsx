...
type CandidateMeProfileResponse = {
  ...
  candidate: {
    publicId: string
    revealed: boolean
    visibilityLog?: Array<{ ts: string; assessmentId?: string; assessmentTitle?: string }>
  }
}
...
export default function ProfilePage() {
  ...
  const [profile, setProfile] = React.useState<CandidateMeProfileResponse | null>(null)
  ...
  async function load() {
    ...
    try {
      ...
      const profileRes = await fetchJson<CandidateMeProfileResponse>('/api/candidate/me/profile')
      ...
      <Container maxW="5xl" py={{ base: 10, md: 14 }}>
        <HStack justify="space-between" align="start" spacing={6} flexWrap="wrap">
          ...
          <HStack spacing={3} pt={2}>
            ...
            <Button onClick={save} isLoading={isSaving} colorScheme="green" bg="brand.600" _hover={{ bg: 'brand.700' }}>
              Save
            </Button>
          </HStack>
        </HStack>
        ...
        {error ? (
          <Box borderWidth="1px" borderColor="red.200" bg="red.50" borderRadius="lg" px={4} py={3} mb={6}>
        ...
        {success ? (
          <Box borderWidth="1px" borderColor="green.200" bg="green.50" borderRadius="lg" px={4} py={3} mb={6}>
            ...
            {profile?.candidate?.visibilityLog?.length ? (
              <Stack spacing={3}>
                {profile.candidate.visibilityLog
                  ...
                  .reverse()
                  .map((v) => (
                    <Box key={`${v.ts}:${v.assessmentId ?? 'na'}`} borderWidth="1px" borderColor="blackAlpha.100" borderRadius="xl" p={4}>
                      <Text fontWeight={700}>{v.assessmentTitle || v.assessmentId || 'Assessment'}</Text>
