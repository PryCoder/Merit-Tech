...
export default function AssessmentSolvePage() {
  ...
  const [assessment, setAssessment] = React.useState<GetAssessmentResponse['assessment'] | null>(null)
  ...
  React.useEffect(() => {
    ...
    async function loadAssessment() {
      setError(null)
      try {
        const res = await fetchJson<GetAssessmentResponse>(`/api/assessments/${encodeURIComponent(assessmentId)}`)
  ...
  function submit() {
    ...
    router.push(`/assessment/${assessmentId}/results?sessionId=${encodeURIComponent(sessionId)}`)
  }
  ...
  return (
    <Box minH="100dvh" bg="var(--background)" color="var(--foreground)">
      <Container maxW="7xl" py={{ base: 10, md: 14 }}>
        <HStack justify="space-between" align="start" spacing={6} flexWrap="wrap">
          <Box>
            ...
            <Text mt={2} color="blackAlpha.700" maxW="80ch">
              Session: <Box as="span" fontFamily="var(--font-geist-mono)">{sessionId || '—'}</Box> · Time elapsed:{' '}
              <Box as="span" fontFamily="var(--font-geist-mono)">{elapsedSec}s</Box>
            </Text>
          </Box>
          ...
          <HStack spacing={3} pt={2}>
            ...
            <Button onClick={submit} colorScheme="green" bg="brand.600" _hover={{ bg: 'brand.700' }} isDisabled={!sessionId}>
              Submit
            </Button>
          </HStack>
        </HStack>
        ...
        {!sessionId ? (
          <Box borderWidth="1px" borderColor="orange.200" bg="orange.50" borderRadius="lg" px={4} py={3} mb={8}>
        ...
        <Stack spacing={8}>
            ...
            {assessment?.tasks?.length ? (
              <>
                ...
                {assessment.tasks.map((t, idx) => (
                  <Box key={t.id || `${idx}:${t.title}`} borderWidth="1px" borderColor="blackAlpha.100" borderRadius="xl" p={4} mb={idx === assessment.tasks.length - 1 ? 0 : 4}>
                    ...
                    {t.exampleInput || t.exampleOutput ? <Divider my={4} /> : null}

                    {t.exampleInput ? (
                      <Box>
                        <Text fontWeight={700} fontSize="sm" color="blackAlpha.700">
                          Example input
                        </Text>
                        <Box mt={2} bg="blackAlpha.50" borderWidth="1px" borderColor="blackAlpha.100" borderRadius="lg" p={3}>
                          <Text fontFamily="var(--font-geist-mono)" fontSize="sm" whiteSpace="pre-wrap">
                            {t.exampleInput}
                          </Text>
                        </Box>
                      </Box>
          ...
          <HStack align="start" spacing={6} flexWrap={{ base: 'wrap', lg: 'nowrap' }}>
            <Box flex={1} minW={{ base: '100%', lg: '55%' }} bg="white" borderRadius="2xl" borderWidth="1px" borderColor="blackAlpha.100" p={{ base: 6, md: 8 }}>
              <Heading as="h2" fontSize="2xl">
                Code editor (MVP)
              </Heading>
            ...
            <Box flex={1} minW={{ base: '100%', lg: '45%' }} bg="white" borderRadius="2xl" borderWidth="1px" borderColor="blackAlpha.100" p={{ base: 6, md: 8 }}>
              <Heading as="h2" fontSize="2xl">
                Mentor (MVP)
              </Heading>
              <Text color="blackAlpha.700" mt={1}>
              ...
              <VStack align="stretch" spacing={3}>
                <Input value={mentor} onChange={(e) => setMentor(e.target.value)} placeholder="Ask a question…" />
