      ...
      <Container maxW="5xl" py={{ base: 10, md: 14 }}>
        <HStack justify="space-between" align="start" spacing={6} flexWrap="wrap">
          ...
          <HStack spacing={3} pt={2}>
            ...
            <Button as={NextLink} href={`/assessment/${assessmentId}/setup`} colorScheme="green" bg="brand.600" _hover={{ bg: 'brand.700' }}>
              Retry (new session)
            </Button>
          </HStack>
        </HStack>
        ...
        <Stack spacing={6}>
          <Box bg="white" borderRadius="2xl" borderWidth="1px" borderColor="blackAlpha.100" p={{ base: 6, md: 8 }}>
            ...
            <Text color="blackAlpha.700">
              Assessment: <Box as="span" fontFamily="var(--font-geist-mono)">{assessmentId}</Box>
            </Text>
            <Text color="blackAlpha.700" mt={2}>
              Session: <Box as="span" fontFamily="var(--font-geist-mono)">{sessionId || '—'}</Box>
            </Text>
            <Text color="blackAlpha.600" mt={4} fontSize="sm">
              Next step: wire this to a backend “get session results” endpoint and render merit score, AI dependency index,
              test cases, and mentor feedback.
            </Text>
          </Box>
        </Stack>
      </Container>
