...
function getCandidate(req, res, next) {
  try {
    const result = candidateService.getCandidateProfileByPublicId(req.params.publicId);
    res.json(result);
  } catch (err) {
    next(err);
  }
...
function revealCandidate(req, res, next) {
  try {
    const candidate = candidateService.revealCandidateByPublicId(req.params.publicId, {
